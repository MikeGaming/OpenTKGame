...
vec4 a_Color = vec4(1.0, 1.0, 1.0, 1.0); // Default color, can be overridden by vertex color if needed
...
void main() {
    ...
    vec3 n = fbm(noise_pos.xy);

    // Adjust height of the vertex by fbm result scaled by final desired amplitude
    pos.z += _TerrainHeight * n.x + _TerrainHeight - _Offset.z;

    vec4 newPos = vec4(pos, 1.0) * model;
