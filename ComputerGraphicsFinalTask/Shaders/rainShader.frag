...
void main() {
    ...
    v = (gl_FragCoord.xy*.1)/ resolution.xy,
    n = noise(v*200.); // Displacement

    //fragColor = texture(texture0, u, 2.5);
    //fragColor.a = 0.1f;

    // Loop through the different inverse sizes of drops
    for (float r = 4. ; r > 0. ; r--) {
        vec2 x = resolution.xy * r * .015,  // Number of potential drops (in a grid)
        p = 6.28 * u * x + (n - .5) * 2.,
        s = sin(p);
        ...
        if (d.r < (5.-r)*.08 && t > .5) {
            ...
            fragColor = texture(texture0, u - v.xy * .3);
            fragColor.a = 0.2f;
        }
    }
